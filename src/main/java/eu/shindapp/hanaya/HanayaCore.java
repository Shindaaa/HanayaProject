    ...
    public static void main(String[] args) {
        ...
        if (new ConfigUtils().getBoolean("mysql-enabled")) {
            try {
                ...
                connectionSource = new JdbcConnectionSource("jdbc:mariadb://" + new ConfigUtils().getString("mysql-host") + ":" + new ConfigUtils().getString("mysql-port") + "/" + new ConfigUtils().getString("mysql-database") + "?autoReconnect=true&wait_timeout=172800", new ConfigUtils().getString("mysql-username"), new ConfigUtils().getString("mysql-password"));
